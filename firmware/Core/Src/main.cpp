...
uint8_t fuse_currents[IC_COUNT]
                     [CHANNEL_COUNT]; // Stored in 0.1A units (hMA) | IC index 0
                                      // = fuse1 (bottom), 3 = fuse4 (top)
...
uint32_t total_current_calc(uint8_t fuse_currents[IC_COUNT][CHANNEL_COUNT]) {
  ...
  for (uint8_t ic = 0; ic < IC_COUNT; ic++) {
    for (uint8_t ch = 0; ch < CHANNEL_COUNT; ch++) {
      sum +=
          fuse_currents[ic][ch]; // If channel is off, current is 0 → no effect
    }
  }
  return sum;
}
...
// old function
// void handle_overcurrent(uint8_t ic_index, uint8_t channel_number, uint8_t
// threshold)
//{
...
//    	        channel_states[ic_index] &= ~(1 << channel_number); // IC index
//    0 = fuse1 (bottom), 3 = fuse4 (top)
...
//    // OUT command
//
//    	        HAL_GPIO_WritePin(SPI1_SS_GPIO_Port, SPI1_SS_Pin,
//    GPIO_PIN_RESET); 	        HAL_SPI_TransmitReceive(&hspi1, tx_buffer,
//    rx_buffer, 5, 100); 	        HAL_GPIO_WritePin(SPI1_SS_GPIO_Port,
//    SPI1_SS_Pin, GPIO_PIN_SET);
//
//    }
//}
...
SystemStatus get_system_status_from_channels() {
  ...
  // FAN: IC1 ch0, ch2
  status.fan_status =
      reduce_status(get_channel_status(0, 0), get_channel_status(0, 3));

  // PUMP: IC1 ch1, ch3
  status.pump_status =
      reduce_status(get_channel_status(0, 1), get_channel_status(0, 2));
  // PC: IC0 ch0-3
  ...
  status.inverter_status =
      reduce_status(get_channel_status(3, 0), get_channel_status(3, 1));
...
int main(void) {
  ...
  MX_SPI1_Init();
  /* USER CODE BEGIN 2 */
  auto st1 = HAL_FDCAN_ActivateNotification(&hfdcan1,
                                            FDCAN_IT_RX_FIFO0_NEW_MESSAGE, 0);
  //  st1 = HAL_FDCAN_ActivateNotification(&hfdcan1,
  //  FDCAN_IT_RX_FIFO0_MESSAGE_LOST, 0); st1 =
  //  HAL_FDCAN_ActivateNotification(&hfdcan1, FDCAN_IT_RX_FIFO0_FULL, 0);

  static FDCAN_FilterTypeDef sFilterConfig = {.IdType = FDCAN_STANDARD_ID,
                                              .FilterIndex = 0,
  ...
  while (1) {
    ...
    fuse_currents[3][2] = mv_to_hma2(__HAL_ADC_CALC_DATA_TO_VOLTAGE(
        __VREFANALOG_VOLTAGE__, adc_buffer[0], ADC_RESOLUTION12b)); // IC3
    Ch2current = (__HAL_ADC_CALC_DATA_TO_VOLTAGE(
        __VREFANALOG_VOLTAGE__, adc_buffer[3], ADC_RESOLUTION12b));
    ...
    //	  if (any_channel_closed && (HAL_GetTick() - last_shutdown_time >=
    // 5000))
    //	  {
    ...
    //	          {
    //	              fuse_currents[ic][ch] =
    // mv_to_hma(__HAL_ADC_CALC_DATA_TO_VOLTAGE(
    //	                  __VREFANALOG_VOLTAGE__, adc_buffer[adc_index],
    // ADC_RESOLUTION12b));
    //
    //	              if (fuse_currents[ic][ch] <= thresholds[ic][ch])
    ...
    //
    //	          HAL_GPIO_WritePin(SPI1_SS_GPIO_Port, SPI1_SS_Pin,
    // GPIO_PIN_RESET); 	          HAL_SPI_TransmitReceive(&hspi1,
    // tx_buffer, rx_buffer, 5, 100); HAL_GPIO_WritePin(SPI1_SS_GPIO_Port,
    // SPI1_SS_Pin, GPIO_PIN_SET);
    //	      }
    //	  }
