...
struct Can_rx_message {
    Can_rx_message(FDCAN_HandleTypeDef& hcan, uint32_t RxFifo) : header{}, data{0} {
        this->status = HAL_FDCAN_GetRxMessage(&hcan, RxFifo, &this->header, this->data);
    }
...
template <typename T> class Can_tx_message {
  public:
    ...
    constexpr Can_tx_message(const T& data, const FDCAN_TxHeaderTypeDef& message_header) : header{message_header} {
    ...
    HAL_StatusTypeDef send(FDCAN_HandleTypeDef& hcan) { return HAL_FDCAN_AddMessageToTxFifoQ(&hcan, &this->header, this->buff); }
};

#else

struct Can_rx_message {
    Can_rx_message(FDCAN_HandleTypeDef& hcan) : header{}, data{0} { this->status = HAL_FDCAN_GetRxMessage(&hcan, FDCAN_RX_FIFO0, &this->header, this->data); }
...
template <typename Device_data_type> class __attribute__((packed)) Device : public Device_base {
  public:
    explicit constexpr Device(uint32_t ide) : Device_base(ide, sizeof(Device_data_type)) { static_assert(sizeof(Device_data_type) <= 8); };
